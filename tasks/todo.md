...
Plan item 11 [ ] Revert `requirements.txt` from `anthropic==0.100.0` back to `openai==2.34.0`.
Plan item 12 [ ] Revert `pipeline/explainer.py` from `ANTHROPIC_API_KEY`/`Anthropic` back to `OPENAI_API_KEY`/`OpenAI`.
